...
| 表記 | 使用する場面 |
| --- | --- |
| ネットワーク機器連携カタログ | SingleID 管理者ポータルで、連携するネットワーク機器を選択するカタログを指す場合 |
| ネットワーク機器連携の設定例 | 公開ドキュメントに掲載するネットワーク機器の設定例を指す場合 |
| ネットワーク機器ベンダー | 無線LANアクセスポイント、ネットワークスイッチ、VPN装置などを提供するベンダーを指す場合 |

`ネットワーク機器カタログ`、`ネットワーク機器連携ガイド`、`ネットワークベンダー`は、画面名や引用を除き、新規文書では使用しません。
