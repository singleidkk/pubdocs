...
!!! warning
    本手順のVPN接続端末は、Windows端末を前提としています。スマートフォンやタブレットは、ソフトウェアトークンアプリを表示する端末として利用できます。
    ...
    * PAP認証を利用できること
    ...
    YAMAHA RTXでRADIUS連携によるリモートアクセスVPNを利用する場合、VPN方式はL2TP/IPsecです。また、本構成ではYAMAHA RTX側のユーザ認証方式としてPAPを選択します。L2TP/IPsecまたはPAP認証を利用できないVPNクライアントは対象外です。
...
4. **基本情報**タブに、以下を設定します。
    ...
    | **IP or ホスト名** | **YAMAHA RTX**側の**グローバルIPアドレス**です。インターネットに出ていくときの送信元のIPアドレスです。グローバルIPアドレスが、動的の場合には、DDNS（ダイナミックDNS）を利用して**ホスト名（FQDN）**を設定します。 |
