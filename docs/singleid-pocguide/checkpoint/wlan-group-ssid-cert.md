...
5. **許可グループ**の設定で**許可したいグループ**[（参照）](#グループの情報)をダブルクリックし、**許可**へ移動させます。
6. **許可**へ移動させたグループの同じ行に、接続を許可したい無線LANアクセスポイントのSSID[（参照）](#グループの情報)を入力します。
7. **登録**ボタンをクリックします。
...
#### Check Point FWの無線LANアクセスポイントへ接続（Windows 10/11 の場合）
