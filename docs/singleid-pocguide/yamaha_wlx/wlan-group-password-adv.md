...
4. **基本情報**タブに、以下を設定します。
    ...
    | **属性値** | RADIUS認証ログで確認したNAS-Identifierをもとに設定します。すべてのYAMAHA WLXシリーズを1つのRADIUSサイトで扱う場合は、`WLX`を設定します。 |

    !!! info
        YAMAHA WLXの場合には、NAS-Identifierは、IPアドレス、モデル名、機器特有の文字列で構成されています。
        例: 192.168.200.100/201-WLX413_Z0000000AL
        属性値に`WLX`を設定すると、DHCPによるIPアドレスの変更や機器交換の際にSingleID側の設定変更が不要となります。
...
3. 以下を設定します。
    ...
    | **プライマリRADIUSポート番号** | **SingleID 管理者ポータル＞認証＞RADIUS＞基本情報**タブの**拡張RADIUSサーバ＞RADIUSポート番号**のポート番号です。 |
