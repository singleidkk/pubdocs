...
4. **基本情報**タブに、以下を設定します。
    ...
    | **属性値** | 任意の文字列を設定します。（例: aruba_instant_ap）<br>ここで設定した文字列を[Aruba Instant 無線LANアクセスポイントの設定](#aruba-instant-無線lanアクセスポイントの設定)の手順で設定するNAS-Identifierにも設定します。<br>**サイト識別する属性**に**NAS-Identifier**を選択した場合、実際にネットワーク機器から送信される属性値に対して、ここの設定項目で設定した文字列で**部分一致検索**します。 |
...
4. プライマリRADIUSサーバを以下の内容で登録します。
    ...
    | **IP Address** | SingleIDの拡張RADIUSサーバのプライマリIPアドレスです。 |
    | **RadSec** | 無効にします。 |
    | **Auth port** | SingleIDの拡張RADIUSサーバに割り当てられたRADIUSポート番号です。（デフォルト値: 1812） |
    | **Shared key** | SingleIDの拡張RADIUSサーバに設定したシークレットです。 |
