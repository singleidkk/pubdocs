...
3. 以下を設定します。
    ...
    | **WPA暗号** | AES |
    | **認証サーバー設定** ||
    | **IPアドレス** | **SingleID 管理者ポータル＞認証＞RADIUS＞基本情報**画面の**拡張RADIUSサーバ＞IPアドレス**の**プライマリ**です。 |
...
4. **基本情報**タブに、以下を設定します。
    ...
    !!! info
        * **NAS-ID**として`AR-WF6-254I`のような値が送信される場合があります。ただし、ハードウェアリビジョンや機種差分により末尾が異なる可能性があるため、初期値としては共通接頭辞である`AR-WF`を設定することをお勧めします。
