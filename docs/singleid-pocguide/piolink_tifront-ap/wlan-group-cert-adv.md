...
4. **基本情報**タブに、以下を設定します。
    ...
    | **属性値** | `AR-WF`を設定します。実際にネットワーク機器から送信される**NAS-Identifier**の属性値に対して、ここの設定項目で設定した文字列で**部分一致検索**します。 |

    !!! info
        * **NAS-Identifier**として`AR-WF6-254I`のような値が送信される場合があります。ただし、ハードウェアリビジョンや機種差分により末尾が異なる可能性があるため、初期値としては共通接頭辞である`AR-WF`を設定することをお勧めします。
