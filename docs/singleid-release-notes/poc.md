...
* ユーザポータルで、利用不可の文字をパスワードに設定できてしまう問題を修正しました。
* ユーザポータルで、メールアドレスを変更できないように修正しました。
...
* Entra ID のユーザ/グループに対する SCIM プロビジョニングをサポートしました。

**SCEP**

* 証明書の有効期間の開始日時を現在日時の 5 分前に設定するよう改善しました（Intune による SCEP 証明書配布に関わる問題を修正）。

### 修正

**RADIUS**

* RADIUS 認証ログの表示を高速化しました。
...
* DynDNS 互換 API を実装しました。
* 契約ホスト数・契約年数・アクティベーション機能を実装しました。

### 修正

**証明書**

* クライアント証明書プロファイルにおいて、UPN を SAN に設定する際、UPN ドメインが未設定でもプロファイルを保存できてしまう不具合を修正しました。
* クライアント証明書プロファイルにおいて、SAN に UPN とメールアドレスの両方の属性を同時に追加できない不具合を修正しました。
...
* **管理＞詳細設定**画面に、ユーザ登録通知メールを送信するかどうかを切り替えられる設定を追加しました。

### 修正
**顧客管理者ポータル**

* **ユーザ一覧**画面で複数ユーザを選択した状態で削除ボタンを押すと、選択したユーザがそのまま削除されてしまう不具合を修正しました。

**ネットワーク機器連携カタログ**

* PacketiX VPNカタログで発生していた不具合を修正しました。
...
* DDNSホストのIP更新用APIキーの長さを32文字に統一しました。
* DynDNS互換APIを新たに追加しました。
