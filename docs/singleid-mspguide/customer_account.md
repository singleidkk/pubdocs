...
!!! info
    サポートアカウントの管理者は、SingleID MSP管理者ポータルでは顧客アカウント情報の参照のみ可能です。割り当てられた顧客アカウントのSingleID 管理者ポータルでは、顧客環境の設定や確認を行うことができます。
...
1. （特権MSP/MSPアカウントの管理者）アカウント登録
    ...
    2. **顧客情報**、**契約ユーザ数**、**販売店情報**を入力し、**登録**ボタンをクリックします。
        ...
        !!! info
            ...
            * 顧客アカウントの**会社名**は、最大255文字です。文字種に特別な制限はありません。

    3. 登録された顧客アカウントは、**SingleID MSP管理者ポータル＞管理アカウント＞承認待ちアカウント一覧**画面に表示されます。申請内容の変更または削除については、[承認待ちアカウント管理](pending_account.md)を参照してください。
...
5. 設定した**解約予定日**に、**解約処理が自動で実行**されます。
    ...
    !!! info
        ...
        * 一度設定した**解約予定日**は、解約処理が実行される前であれば変更できます。また、**解約予定日**を空欄にして保存することで解除できます。

### 解約後の処理
顧客アカウントの解約後は、SingleIDで以下の処理を行います。

| **時期** | **処理内容** |
| :--- | :--- |
| 解約予定日の翌営業日から起算して3営業日以内 | すべてのRADIUS認証を無効化し、顧客アカウントに登録されたすべての管理者を削除します。 |
| 解約予定日から1か月後 | 発行済みのクライアント証明書を失効し、ユーザ、ログ、証明書情報など、顧客アカウントのすべてのデータを削除します。 |

!!! info
    * 解約予定日がSingleIDの利用最終日です。
    * 即時解約の場合は、解約を実行した日を解約予定日として扱います。
    * RADIUS認証の無効化時点では、発行済みのクライアント証明書は失効しません。SingleIDのRADIUS認証はできなくなりますが、SingleIDのRADIUSと連携せずにクライアント証明書だけを検証する外部システムでは、証明書が失効するまで認証できる場合があります。

!!! warning
    削除されたデータを復元することはできません。

解約後に契約の継続を希望する場合は、データが削除される前にSingleIDの担当営業またはサポートまでお問い合わせください。

## 契約期限前の警告通知
契約が終了する前に、担当者へ自動で警告通知メールが送信されます。これにより、契約更新の対応漏れを防止できます。
...
上記のタイミングで、自動的に通知メールが送信されます。

!!! warning
    解約予定日が設定されていない場合には通知されません。

### 通知先

通知メールは、顧客アカウントに登録された**担当者Eメール**宛に送信されます。
以下の 2か所の設定が対象です。

* **顧客アカウント編集 ＞ 基本情報タブ ＞ 担当者Eメール**
* **顧客アカウント編集 ＞ 販売店情報タブ ＞ 担当者Eメール**
    
!!! info
    両方にメールアドレスが登録されている場合、**双方へ通知が送信**されます。
...
## サポートアカウント
顧客用のSingleID 管理者ポータルへサポートアカウントの管理者がアクセスできるように、顧客アカウントとサポートアカウントを関連付けます。

1. 顧客アカウントの**編集**ボタンをクリックします。
2. **サポート**タブへ移動します。
3. **サポートアカウント**のプルダウンメニューから顧客アカウントの顧客用のSingleID 管理者ポータルへアクセスを許可したいサポートアカウントを選択します。
4. **更新**ボタンをクリックします。

## 画面のカスタマイズ
顧客用のSingleID 管理者ポータルのメニューの表示/非表示の設定を行います。

1. 顧客アカウントの**編集**ボタンをクリックします。
2. **画面のカスタマイズ**タブへ移動します。
3. **非表示設定**で、顧客用のSingleID 管理者ポータルで非表示にしたいメニューのチェックボックスのチェックを外します。
4. **更新**ボタンをクリックします。
