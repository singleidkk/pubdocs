...
!!! info
    ...
    * **ユーザ新規登録時**および**クライアント証明書発行時**には、ユーザの**メールアドレス**宛に通知メールが送信されますが、**基本情報**タブの**通知メールアドレス**が設定されている場合には、**通知メールアドレス**宛に送信することができます。ユーザのメールアドレスとは異なるメールアドレスに通知したい場合に設定します。
    * **ユーザ名**は最大128文字です。使用できる文字は、半角英字（大文字・小文字）、半角数字、半角記号の`-`、`_`、`@`、`.`です。日本語、全角文字、空白、上記以外の記号は使用できません。
...
1. グループへ追加したいユーザの行にある**チェックボックス**を選択します。（複数選択可能）
2. **選択実行**ボタンの▼をクリックし、プルダウンメニューの**グループへ追加**を選択します。
3. **グループへ追加**画面で、追加先のグループを選択します。
4. **追加**ボタンをクリックして、選択したユーザをグループへ追加します。

### グループからの削除

1. グループから削除したいユーザの行にある**チェックボックス**を選択します。（複数選択可能）
2. **選択実行**ボタンの▼をクリックし、プルダウンメニューの**グループから削除**を選択します。
3. **グループから削除**画面で、削除対象のグループを選択します。
4. **削除**ボタンをクリックして、選択したユーザをグループから削除します。

!!! info
    グループから削除しても、ユーザは削除されません。
...
!!! info
    * **アカウント番号**の列
        ...
        顧客用のSingleID 管理者ポータルのヘッダー部分にある**アカウント番号**（8桁の数字）を定義します。顧客用のSingleID 管理者ポータルのヘッダー部分にある**アカウント番号**とCSVファイルに定義した**アカウント番号**が一致しない場合には、ユーザは登録できません。

    * **パスワードメール送信**の列
    
        * 0：パスワード列で指定した値をそのまま設定します。
        * 1：パスワード再設定メールを送信します。
        * 2：既存ユーザの更新時のみ利用できます。現行のパスワードを保持し、再設定メールも送信しません。新規ユーザ行で 2 を指定するとエラーになります。

    * **グループ**の列
    
        カンマ区切りで複数のグループを定義できます。存在しないグループを定義した場合には、ユーザ登録およびユーザ更新はできません。
