...
このドキュメントでは、Microsoft Intuneを使ってSCEP証明書をデバイスに配布する手順を説明します。  
※ 本手順では、対象デバイスがすでにIntuneに登録済みであることを前提としています。
...
```text
SCEP: 失敗した LogError メッセージ: (SCEPInstallCertificateWithScepHelper:Failed to Initialize SCEP enrollment with NDES Server 'https://scep-02-0001.poc.singleid.jp/scep/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX/pkiclient.exe', CA cert thumbprint 'XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX' ...)
```

上記のログは、Intune の信頼された証明書プロファイルで指定したルートCA/中間CA証明書と、SCEP サーバーから取得した証明書が一致しない場合に発生します。

まず、Intune の構成プロファイルに設定した CA 証明書（ルートCA/中間CA）が正しいか確認してください。
