...
* SingleID から **SCIM Base URL** および **Bearer Token（アクセストークン）** が発行済みであること
...
## 4. SingleID 側の設定（SCIM エンドポイント発行）

SingleID の SCIM 連携は、必要なパラメータを SingleID 側へ提示いただき、SingleID 側で SCIM エンドポイント（SCIM Base URL / Bearer Token）を発行します。
...
* **Bearer Token（シークレット トークン / アクセストークン）**
...
!!! warning
    Bearer Token を再発行した場合、既存の Bearer Token は利用できなくなります。運用中に再発行する際は、Microsoft Entra ID 側の設定も更新してください。
...
3. **新しいプロビジョニング構成**画面で、以下を設定します。
    ...
    | --- | --- |
    | 認証方法の選択 | **ベアラー認証** |
    | テナントのURL | 本書記載の **SCIM Base URL** |
    | シークレット トークン | SingleID から提供された **Bearer Token** |
