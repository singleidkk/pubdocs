# Microsoft Entra ID（SCIM）による SingleID ユーザ／グループ同期
文書更新日:2026-06-11

## 1. 目的と概要

本マニュアルは、Microsoft Entra ID（旧 Azure Active Directory）の **SCIM プロビジョニング機能** を利用して、**SingleID** にユーザおよびグループを自動同期するための設定手順書です。

本書は以下の利用者を想定しています。

* Microsoft Entra ID 管理者
* SIer / 販売店経由で SingleID を導入するエンドユーザ管理者
...
SingleID では、Microsoft Entra ID の**ユーザー プリンシパル名（UPN）**を元にユーザを識別します（SingleID 側のユーザ名は UPN の`@`より前の部分が利用されます）。

* 同期対象にできる UPN サフィックスは1つであること
* 同期対象ユーザの UPN サフィックスを、SingleID 側へ事前に登録していること

    UPN サフィックスは、UPN の`@`より右側の文字列です。例：`user1@example.com` を同期する場合、`example.com` の登録が必要です。

* 同期開始後に、同期対象ユーザの UPN を変更しない運用にできること

### ユーザ属性の前提（SCIM）

SCIM で SingleID のユーザを作成・更新するため、以下の属性が Microsoft Entra ID 側で設定されていることを事前に確認してください。未設定（空値）の場合、対象ユーザのプロビジョニングは失敗します。

| SingleID の必須項目 | SCIM 属性 | Microsoft Entra ID 属性（デフォルト） |
| --- | --- | --- |
| ユーザ名 | `userName` | `userPrincipalName`（`@` より前の部分を利用） |
| 名 | `name.givenName` | `givenName` |
| 姓 | `name.familyName` | `surname` |
...
Microsoft Entra ID Free については、2026年6月11日時点で、ユーザプロビジョニングが動作することを確認しています。ただし、グループプロビジョニングが利用できないため、本書の前提（ユーザ／グループ同期）を満たしません。また、無償版の仕様は Microsoft 側で変更される可能性があるため、本番運用では Microsoft Entra ID P1 以上の利用を推奨します。
...
3. 以下を設定し、**保存**をクリックします。
    ...
    !!! info
        ...
        * 本アプリは SCIM 同期用途のため、エンドユーザのサインインや表示は不要です。
...
### 6.1 ユーザ属性マッピング（Users）

マッピングは、サイドメニューの**属性マッピング（プレビュー）**から設定します。

1. サイドメニューから**属性マッピング（プレビュー）**をクリックします。
2. 一覧から **Provision Microsoft Entra ID Users** をクリックします。Microsoft Entra ID のユーザー属性を SingleID の SCIM ユーザ属性にマッピングします。SingleID 連携では、基本的に**デフォルトのマッピングのままで動作します**。以下はデフォルトのマッピング例です。
...
### 8.0 SingleIDへ同期されるユーザ項目

SingleID に同期される主なユーザ項目は以下です。

* ユーザ名：Microsoft Entra ID の**ユーザー プリンシパル名（UPN）**の`@`より前の部分
* 名：`givenName`
* 姓：`surname`
* メールアドレス：`mail`
* 携帯電話：`mobile`
...
SingleID では、Microsoft Entra ID から同期されるユーザを **ユーザー プリンシパル名（UPN）** を元に識別します。SingleID 側のユーザ名は、UPN の `@` より前の部分が利用されます。

UPN の `@` より右側の UPN サフィックスが SingleID 側に登録されていない場合、そのユーザの SCIM 同期は失敗します。

!!! warning
    UPN を変更すると、SingleID 側のユーザ名と一致しなくなり不整合が発生します。運用上、同期対象ユーザの UPN は変更しないでください。
...
| ユーザー追加 | ユーザ作成 |
| ユーザー属性変更 | ユーザ更新 |
| ユーザー割当解除 / ユーザー無効化 / ユーザー論理削除 | ユーザ無効化 |
| ユーザー物理削除 | ユーザ削除 |
| グループ作成 | グループ作成 |
...
### 8.3 SCIM で作成されたユーザのパスワードと登録通知メール

* SCIM プロビジョニングでは、Microsoft Entra ID のパスワードは SingleID へ同期されません。そのため、SCIM プロビジョニングにより SingleID ユーザが作成された場合、ユーザのメールアドレス宛へパスワード設定の案内が送信されます。
* **ユーザの登録通知**のメールは、**SingleID 管理者ポータル＞管理＞詳細設定**画面の**ユーザに関する設定＞登録時にメール通知**の設定に従います。
...
| 事象 | 原因 | 確認ポイント |
| --- | --- | --- |
| ユーザー／グループが同期されない | 割り当て漏れ | アプリにユーザー／グループが割り当てられているか |
| ユーザ作成/更新が失敗する | UPN サフィックス未登録 | 同期対象ユーザの UPN サフィックスが SingleID 側に登録されているか |
| ユーザ作成/更新が失敗する | 必須属性が未設定 | `userPrincipalName` / `givenName` / `surname` / `mail` の値とマッピングを確認 |
| メールアドレスが反映されない | 属性マッピング不備 | `mail` が `emails[type eq "work"].value` にマッピングされているか |
| グループメンバーが欠落する | ネストグループ | 割り当てたグループの直下にユーザが含まれているか |

---

## 11. 初回セットアップ時の推奨手順

* 最初は **少数ユーザ（1〜2名）で接続テスト** を行う
