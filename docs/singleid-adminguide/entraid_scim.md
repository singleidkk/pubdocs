...
* 同期対象ユーザーの UPN サフィックスを、SingleID 側へ事前に登録していること
...
SCIM で SingleID のユーザーを作成・更新するため、以下の属性が Microsoft Entra ID 側で設定されていることを事前に確認してください。未設定（空値）の場合、対象ユーザーのプロビジョニングは失敗します。
...
Microsoft Entra ID Free については、2026年6月11日時点で、ユーザー プロビジョニングが動作することを確認しています。ただし、グループ プロビジョニングが利用できないため、本書の前提（ユーザー／グループ同期）を満たしません。また、無償版の仕様は Microsoft 側で変更される可能性があるため、本番運用では Microsoft Entra ID P1 以上の利用を推奨します。
...
UPN の `@` より右側の UPN サフィックスが SingleID 側に登録されていない場合、そのユーザーの SCIM 同期は失敗します。
...
| ユーザー作成/更新が失敗する | UPN サフィックス未登録 | 同期対象ユーザーの UPN サフィックスが SingleID 側に登録されているか |
| ユーザー作成/更新が失敗する | 必須属性が未設定 | `userPrincipalName` / `givenName` / `surname` / `mail` の値とマッピングを確認 |
| メールアドレスが反映されない | 属性マッピング不備 | `mail` が `emails[type eq "work"].value` にマッピングされているか |
| グループメンバーが欠落する | ネストグループ | 割り当てたグループの直下にユーザーが含まれているか |
