...
7. **URL**に、SonicWall Cloud Secure EdgeのURLを設定します。（例：`https://SonicWall Cloud Secure Edge.example.com`形式 または　`https://SonicWall Cloud Secure Edge.example.com/index.php`形式 ）
8. **IdP エンドポイントメタデータ**の**ダウンロード**ボタンをクリックし、メタデータを取得します。このメタデータは、SonicWall Cloud Secure Edgeの設定を行うときに必要となります。
...
1. SonicWall Cloud Secure Edgeへ管理者権限を持つユーザでログインします。
...
6. 以下の内容で設定し、**更新**をクリックします。
    ...
    | :--- | :--- |
    | **グローバル設定** | :fontawesome-regular-square-check:SonicWall Cloud Secure Edge デスクトップクライアントにSAML認証を使用する（ユーザーの再認証が必要）<br>:fontawesome-regular-square-check:複数のユーザーのバックエンド（LDAPなど）の使用を許可する |
    | **一般** ||
...
**SonicWall Cloud Secure Edgeのシステム管理者の権限をSingleIDのグループのメンバーに付与したい場合**には、SingleIDに**admin**という名前のグループを作成し、権限を付与したいユーザをメンバーとして作成した**admin**グループに追加してください。
...
## 動作確認
1. SonicWall Cloud Secure Edgeのログイン画面へアクセスします。
2. **SingleID ログイン**ボタンが追加されていることを確認します。
    
    [![Screenshot](/images/image-10.png)](/images/image-10.png)
    
3. **SingleID ログイン**ボタンをクリックします。
4. SingleIDのログイン画面が表示されます。すでに、SingleIDへログイン中であれば、ログイン画面は表示されず、SonicWall Cloud Secure Edgeへログインできます。
    
    [![Screenshot](/images/image-7-1024x462.png)](/images/image-7-1024x462.png)

## ダイレクトログインの無効化
!!! danger
    以下は、ダイレクトログインを無効化する方法です。必ずSAML認証により**管理者権限**でログインできることを確認できてから、実施するようにしてください。SAML認証ができない状態で、ダイレクトログインを無効化するとSonicWall Cloud Secure Edgeへログインできなくなります。
